# Builds the logos-waku-ui-app standalone application
{ pkgs, common, src, logosLiblogos, logosSdk, logosWakuModule, logosCapabilityModule, logosWakuUI }:

pkgs.stdenv.mkDerivation rec {
  pname = "logos-waku-ui-app";
  ...
  preFixup = ''
    ...
    find $out -type f -executable -exec sh -c '
      if file "$1" | grep -q "ELF.*executable"; then
        ...
        if echo "$1" | grep -q "/logos-waku-ui-app$"; then
  ...
  configurePhase = ''
    ...
    echo "Configuring logos-waku-ui-app..."
    ...
    test -d "${logosWakuUI}" || (echo "waku-ui not found" && exit 1)
  ...
  buildPhase = ''
    ...
    cmake --build build
    echo "logos-waku-ui-app built successfully!"
  ...
  installPhase = ''
    ...
    if [ -f "build/bin/logos-waku-ui-app" ]; then
      cp build/bin/logos-waku-ui-app "$out/bin/"
      echo "Installed logos-waku-ui-app binary"
    fi
    ...
    # Copy waku_ui Qt plugin to root directory (not modules, as it's loaded differently)
    if [ -f "${logosWakuUI}/lib/waku_ui.$OS_EXT" ]; then
      cp -L "${logosWakuUI}/lib/waku_ui.$OS_EXT" "$out/"
    fi

    # Create a README for reference
    cat > $out/README.txt <<EOF
Logos Waku UI App - Build Information
=====================================
...
Runtime Layout:
- Binary: $out/bin/logos-waku-ui-app
- Libraries: $out/lib
- Modules: $out/modules
- Qt Plugin: $out/waku_ui.$OS_EXT

Usage:
  $out/bin/logos-waku-ui-app
EOF
